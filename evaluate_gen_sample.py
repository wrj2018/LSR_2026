...
for material in tqdm(range(12)): # evaluating the material parameters -> `y_labels`
    matl_spearman_coeff_mat, matl_pval_coeff_mat = np.zeros((6, 12)), np.zeros((6, 12))
    stress_dat = decoded_samples[material, :, 12:]; material_dat = decoded_samples[material, :, :12] # util: line->198
...
tmp_stress_spearman_coeff_mat = np.zeros((6, 6))
